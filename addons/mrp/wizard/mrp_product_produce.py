...
class MrpProductProduce(models.TransientModel):
    ...
    produce_line_ids = fields.One2many(
        "mrp.product.produce.line", "product_produce_id", string="Product to Track"
    )
    ...
    def do_produce(self):
        ...
        if (
            float_compare(quantity, 0, precision_rounding=self.product_uom_id.rounding)
            <= 0
        ):
            raise UserError(
    ...
    def check_finished_move_lots(self):
        ...
        if produce_move and produce_move.product_id.tracking != "none":
            ...
            if existing_move_line:
                ...
                produced_qty = self.product_uom_id._compute_quantity(
                    self.product_qty, existing_move_line.product_uom_id
                )
                existing_move_line.product_uom_qty += produced_qty
                existing_move_line.qty_done += produced_qty
            else:
                location_dest_id = (
                    produce_move.location_dest_id.get_putaway_strategy(
                        self.product_id
                    ).id
                    or produce_move.location_dest_id.id
                )
                vals = {
            ...
            if pl.qty_done:
                ...
                if not pl.move_id:
                    ...
                    move_id = self.production_id.move_raw_ids.filtered(
                        lambda m: m.product_id == pl.product_id
                ...
                pl.move_id._generate_consumed_move_line(
                    pl.qty_done, self.lot_id, lot=pl.lot_id
                )
        return True
...
class MrpProductProduceLine(models.TransientModel):
    ...
    qty_done = fields.Float(
        "Consumed", digits=dp.get_precision("Product Unit of Measure")
    )
